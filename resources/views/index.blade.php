...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            input.balance, p.balance {
                outline: 2px solid #000;
                background: #ADFF2F;
                width: 200px;
            }

            input.balance_partner, p.balance_partner {
                outline: 2px solid #000;
                background: #00CED1;
                width: 200px;
            }

            input.balance_cashback , p.balance_cashback{
                outline: 2px solid #000;
                background: #F4A460;
                width: 200px;
            }
    ...
    <body class="antialiased">
        ...
        <form  id="transfer" method="POST" action="/from_user">
            ...
            <div style="color: red;" class="alert alert-danger">{{ $message }}</div>
            @enderror
            <p><input class="balance" type="submit" value="Перевести"></p>
        </form>

        <p class="balance_site">Баланс сайта</p>
        <p class="balance_site">{{ $siteBalance }}</p>

        <form id="transfer_partner" method="POST" action="/to_partner">
            @csrf
            <p class="balance_partner"><label for="balance_partner">Баланс партнера</label></p>
            <p><input   id="transfer_partner"  class="balance_partner"  type="text"  name="balance_partner" value="{{ $partnerBalance }}" readonly></p>
            <p><input class="balance_partner"  type="submit" value="Забрать"></p>
        </form>

        <form  id="transfer_cashback" method="POST" action="/to_cashback">
            @csrf
